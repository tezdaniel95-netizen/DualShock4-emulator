...
3. Распаковать и запустить DualShock4 emulator (**Внимание!** Важно запускать DS4 emulator до запуска `PS Plus`, если вы используете Xbox контроллер, чтобы программа `PS Plus` отдала приоритет DualShock 4 контроллеру).
...
## Клавиши для эмуляции тачпада, движений и т.д.

DualShock 4 | Клавиатура и мышь
------------ | -------------
Touchpad свайп вверх, вниз, влево, вправо | `Home, End, Delete, Page down`
Touchpad первое прикосновение: вверх, вниз, влево, вправо, вниз | `U, J, H, K`
Touchpad второе прикосновение: вверх, вниз, влево, вправо, вниз | `↑, ↓, ←, →`
Потрясти геймпад | `T`
Повернуть геймпад вперед, назад, вправо, влево (гироскоп, датчик движения) | `Numpad 8, 2, 4, 6, 7, 9`
PS | F2

В конфигурационном файле профиля `Default.ini`, в папке `Profiles`, можно изменить привязки кнопок.
...
Параметры чувствительности `SensX`, `SensY` для мыши можно также найти в конфигурационном файле `Config.ini`, в разделе `Mouse`. При отсуствии движения стика можно попробовать увеличить параметр `SleepTimeOut` до 2, 4, 8, 10.
